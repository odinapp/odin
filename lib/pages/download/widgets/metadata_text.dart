...
class _MetadataText extends StatelessWidget {
  const _MetadataText({Key? key, required this.color}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(
        vertical: 12.0.toAutoScaledHeight,
        horizontal: 64.0.toAutoScaledWidth,
      ),
      child: SizedBox(
        ...
        child: Text(
          Provider.of<DioNotifier>(context).miniApiStatus == ApiStatus.failed
              ? Provider.of<DioNotifier>(
                      context,
                    ).fetchFilesMetadataFailure?.message ??
                    'Invalid token'
