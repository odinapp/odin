...
class EncryptionService {
  ...
  Future<EncryptedFile> encryptFile(File file) async {
    ...
    final encryptedFilePath = join(
      file.parent.path,
      '${basename(file.path)}.odin',
    );
  ...
  Future<File> decryptFile(EncryptedFile encryptedFile) async {
    ...
    crypt.setPassword(password);
    crypt.setOverwriteMode(AesCryptOwMode.rename);
    file = await file.rename(
      "${file.path.substring(0, file.path.length - 5)}.aes",
    );
    final decryptedFilePath = crypt.decryptFileSync(
      file.path,
      join(file.parent.path, basenameWithoutExtension(file.path)),
    );
